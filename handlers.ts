...
function shouldSendToSecondAdmin(senderId: number, receiverId: number): boolean {
  // Если отправитель ИЛИ получатель - главный админ, НЕ отправляем второму админу
  if (senderId === MAIN_ADMIN_ID || receiverId === MAIN_ADMIN_ID) {
    return false;
  }
  return true;
}
...
async function sendToBothAdmins(ctx: Context, message: string, options?: any) {
  try {
    ...
    // Второму админу отправляем только если не участвует главный админ
    if (ctx.from?.id !== SECOND_ADMIN_ID && shouldSendToSecondAdmin(ctx.from?.id || 0, ctx.businessConnection?.user_chat_id || 0)) {
